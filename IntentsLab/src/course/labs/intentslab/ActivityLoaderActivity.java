...
public class ActivityLoaderActivity extends Activity {
	...
	private void startExplicitActivation() {
		...
		Intent intentExplicit = new Intent (getApplicationContext(), ExplicitlyLoadedActivity.class);
		// Start an Activity using that intent and the request code defined above
		startActivityForResult(intentExplicit, GET_TEXT_REQUEST_CODE);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == GET_TEXT_REQUEST_CODE){
			if (resultCode == RESULT_OK){
				mUserTextView.setText(data.getStringExtra(ExplicitlyLoadedActivity.EXTRA_USER_INPUT).toString());
			}
		}
